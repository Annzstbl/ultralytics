...
model_cfg = '/data/users/litianhao/hsmot_code/ultralytics/ultralytics/cfg/models/11/yolo11l-obb-8ch-convmsi.yaml'

experiment_name = '197/yolov11l_8ch_CocoPretrain_convhsi_imgsize1280_fullFirstLr10_1gpu2' 


# model = YOLO(model_cfg).load(load_convhsi_pt(pt_file, pt_file.replace('.pt', 'convhsi.pt')))
model = YOLO(model_cfg).load(pt_file.replace('.pt', 'convhsi.pt'))
